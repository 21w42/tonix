...
contract AssemblyLine is Common, Format {
    ...
    function _assemble_standard_device(uint8 n) internal returns (address addr) {
        ...
        addr = _assemble_device(n, version, construction_cost, description, block_size, n_blocks, si);
        _system[n] = addr;
    }

    function assemble_custom_device(uint8 n, string description, uint16 block_size, uint16 n_blocks) external accept {
        (uint8 version, uint16 construction_cost, , , , TvmCell si, ) = _images[n].unpack();
        _assemble_device(n, version, construction_cost, description, block_size, n_blocks, si);
    }

    function _assemble_device(uint8 n, uint8 version, uint16 construction_cost, string description, uint16 block_size, uint16 n_blocks, TvmCell si) internal returns (address addr) {
        ...
        new Base{stateInit: new_si, value: uint64(construction_cost) * 1e9}(dev, address(this));
